...
from custom_components.padspan_bright import telemetry as T
...
def test_the_tab_vocabulary_is_the_panel():
    ...
    www = Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright" / "www" / "padspan-bright"
...
def test_the_resolver_counts_new_resolutions_and_new_unresolved_rpas():
    ...
    reports."""
    from custom_components.padspan_bright.private_ble_resolver import PrivateBLEResolver
    h = _hass()
...
def test_health_reports_how_many_keys_are_resolving():
    from custom_components.padspan_bright import private_ble_resolver as pbr
    h = _hass()
...
def test_a_send_builds_a_snapshot_first(monkeypatch):
    ...
    built = []
    import sys, types
    fake_sb = types.ModuleType("custom_components.padspan_bright.snapshot_builder")
    async def _ls(hass):
        built.append(hass)
        return {}
    fake_sb._live_snapshot = _ls
    monkeypatch.setitem(sys.modules, "custom_components.padspan_bright.snapshot_builder", fake_sb)
...
def test_default_is_off_and_the_wire_is_registered():
    ...
    js = (root / "www" / "padspan-bright" / "views" / "settings.js").read_text(encoding="utf-8")
    assert "padspan_bright/telemetry_preview" in js and "Help improve PadSpan" in js
    panel = (root / "www" / "padspan-bright" / "panel.js").read_text(encoding="utf-8")
...
def test_the_panel_installs_the_error_listeners_and_removes_them():
    from pathlib import Path
    panel = (Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright"
             / "www" / "padspan-bright" / "panel.js").read_text(encoding="utf-8")
...
def test_the_settings_path_is_right_everywhere_it_is_stated():
    ...
    for rel in ("custom_components/padspan_bright/telemetry.py",
                "custom_components/padspan_bright/www/padspan-bright/panel.js",
                "README.md"):
...
def test_the_answer_is_a_setting_the_wire_accepts():
    from pathlib import Path
    ws = (Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright" / "ws_settings.py").read_text(encoding="utf-8")
...
def test_the_panel_asks_in_both_places_and_only_until_answered():
    ...
    assert 'this._callWS({ type: "padspan_bright/telemetry_preview" })' in panel
...
def test_install_base_is_wired_and_gated():
    ...
    assert '"type": "padspan_bright/install_base"' in src
    assert "@websocket_api.require_admin" in src.split("padspan_bright/install_base")[1].split("async def ws_install_base")[0]
    ...
    assert (root / "www" / "padspan-bright" / "views" / "installbase.js").exists()


def test_install_base_refuses_below_pro():
    from custom_components.padspan_bright import ws_telemetry as W
    h = _hass()
    ...
    import custom_components.padspan_bright.licence as L
    orig = L.hass_tier_at_least
    L.hass_tier_at_least = lambda hass, want: False
    try:
        _run(W.ws_install_base(h, conn, {"id": 1, "type": "padspan_bright/install_base"}))
    finally:
