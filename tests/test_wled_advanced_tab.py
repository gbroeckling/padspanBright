...
function fakeHass(writes) {
  return { states: { "light.upper_north": { state: "on", attributes: { friendly_name: "Upper North", effect_list: ["Solid", "Blink"], brightness: 128 } } },
    callService: async () => {},
    callWS: async (m) => {
      if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path] };
      if (m.type === "padspan_bright/wled_state") { writes.push(m.body); return { data: DEVICE["json/si"].state }; }
...
const hass = { states: {}, callWS: async (m) => {
  calls.push(m);
  if (m.type === "padspan_bright/wled_get" && m.device_id) return m.path === "json/cfg" ? { data: cfgs[m.device_id], hash: "h" + m.device_id }
    : { data: { ver: m.device_id === "dL" ? "16.0.1" : "0.15.3", vid: m.device_id === "dL" ? 2607070 : 2503090 } };
  if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path], hash: "hme" };
  if (m.type === "padspan_bright/wled_teams_get") return { teams: [] };
  if (m.type === "padspan_bright/wled_devices") return { devices };
  if (m.type === "padspan_bright/wled_teams_set") return { teams: m.teams };
...
out.cfg = calls.filter(c => c.type === "padspan_bright/wled_cfg").map(c => [c.device_id, c.patch, c.base_hash]);
out.live = calls.filter(c => c.type === "padspan_bright/wled_state" && c.device_id).map(c => [c.device_id, c.body]);
const sets = calls.filter(c => c.type === "padspan_bright/wled_teams_set");
...
const hass = { states: {}, callWS: async (m) => {
  calls.push(m);
  if (m.type === "padspan_bright/wled_get") return { data: m.path === "json/pins" ? [] : DEVICE[m.path], hash: "H" };
  if (m.type === "padspan_bright/wled_cfg") return { backup: "x" };
  return { data: DEVICE["json/si"].state };
} };
...
out.patch = (calls.find(c => c.type === "padspan_bright/wled_cfg") || {}).patch;
""")
...
hass.callWS = async (m) => { calls.push(m.type === "padspan_bright/wled_identify" ? m : null); return orig(m).catch(() => ({})); };
const pane = document.createElement("div");
...
""")
    assert out["call"] == {"type": "padspan_bright/wled_identify", "entity_id": "light.upper_north", "seg_id": 0, "seconds": 10}
...
const hass = { states: {}, callWS: async (m) => {
  calls.push(m);
  if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path], hash: "H" };
  if (m.type === "padspan_bright/wled_cfg") return { backup: "b", after: DEVICE["json/cfg"], hash: "H2", unexpected: [] };
  return { data: DEVICE["json/si"].state };
} };
const pane = document.createElement("div");
await WA.mountWledAdvanced(pane, { hass, eid: "light.upper_north", api: { wled: { isAdmin: true }, toast: () => {} } });
...
await settle();
out.patches = calls.filter(c => c.type === "padspan_bright/wled_cfg").map(c => c.patch);
""")
...
const hass = { states: {}, callWS: async (m) => m.type === "padspan_bright/wled_get" ? { data: m.path === "json/pins" ? [] : DEVICE[m.path], hash: "H" } : {} };
const pane = document.createElement("div");
await WA.mountWledAdvanced(pane, { hass, eid: "light.upper_north", api: { wled: { isAdmin: true }, toast: () => {} } });
await settle();
all(pane).find(n => n.textContent === "LEDs").click();
...
out.unsubs = unsubs;
""")
    assert out["msg"] == {"type": "padspan_bright/wled_live", "entity_id": "light.upper_north"}
    assert out["unsubs"] == 1
...
const hass = { states: {}, callWS: async (m) => {
  ...
  if (m.type === "padspan_bright/wled_teams_get") return { teams: [] };
  if (m.type === "padspan_bright/wled_devices") return { devices };
  if (m.type === "padspan_bright/wled_teams_set") { recorded = m.teams; return { teams: m.teams }; }
  if (m.type === "padspan_bright/wled_cfg") {
  ...
  if (m.type === "padspan_bright/wled_state" && m.device_id === "dA" && !globalThis._aBusyOnce) { globalThis._aBusyOnce = true; const e = new Error("busy"); e.code = "busy"; throw e; }
  return { data: DEVICE["json/si"].state };
} };
globalThis.confirm = () => true;
const pane = document.createElement("div");
...
out.cfgWrites = calls.filter(c => c.type === "padspan_bright/wled_cfg").map(c => [c.device_id, JSON.stringify(c.patch.if.sync.recv.grp), JSON.stringify(c.patch.if.sync.send.grp)]);
...
const hass = { states: {}, callWS: async (m) => {
  ...
  if (m.type === "padspan_bright/wled_get" && m.device_id) return m.path === "json/cfg" ? { data: JSON.parse(JSON.stringify(cfgs[m.device_id])), hash: "h" }
    : { data: { ver: "0.15.3", vid: 2503090 } };
  if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path], hash: "hme" };
  if (m.type === "padspan_bright/wled_teams_get") return { teams: [] };
  if (m.type === "padspan_bright/wled_devices") return { devices };
  if (m.type === "padspan_bright/wled_teams_set") { saves.push(JSON.parse(JSON.stringify(m.teams))); return { teams: m.teams }; }
  if (m.type === "padspan_bright/wled_cfg") {
...
const hass = { states: {}, callWS: async (m) => {
  ...
  if (m.type === "padspan_bright/wled_state" && m.device_id === "dA") { const e = new Error("busy"); e.code = "busy"; throw e; }
  return { data: DEVICE["json/si"].state };
} };
globalThis.confirm = () => true;
const pane = document.createElement("div");
...
hass.callWS = async (m) => m.type === "padspan_bright/wled_cfg" ? { backup: "b", unexpected: ["light.gc.bri"] }
  : m.type === "padspan_bright/wled_get" ? { data: DEVICE[m.path], hash: "H" } : inner(m);
const pane = document.createElement("div");
...
let stored = [], refused = [];
const hass = { states: {}, callWS: async (m) => {
  if (m.type === "padspan_bright/wled_get" && m.device_id) return m.path === "json/cfg"
    ? { data: deep(cfgs[m.device_id]), hash: JSON.stringify(cfgs[m.device_id]) } : { data: { ver: "0.15.3", vid: 2503090 } };
  if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path], hash: "hme" };
  if (m.type === "padspan_bright/wled_teams_get") return { teams: deep(stored), hash: "h" };
  if (m.type === "padspan_bright/wled_devices") return { devices };
  if (m.type === "padspan_bright/wled_teams_set") { stored = deep(m.teams); return { teams: deep(stored), hash: "h" }; }
  if (m.type === "padspan_bright/wled_cfg") {
...
const hass = { states: {}, callWS: async (m) => {
  ...
  if (m.type === "padspan_bright/wled_get" && m.device_id) return m.path === "json/cfg"
    ? { data: deep(cfgs[m.device_id]), hash: JSON.stringify(cfgs[m.device_id]) } : { data: { ver: "0.15.3", vid: 2503090 } };
  if (m.type === "padspan_bright/wled_get") return { data: DEVICE[m.path], hash: "hme" };
  if (m.type === "padspan_bright/wled_teams_get") return { teams: deep(stored), hash: hashOf() };
  if (m.type === "padspan_bright/wled_devices") return { devices };
  if (m.type === "padspan_bright/wled_teams_set") {
  ...
  if (m.type === "padspan_bright/wled_state" && m.device_id) return { data: {} };
  return { data: DEVICE["json/si"].state };
} };
globalThis.confirm = () => true;
const pane = document.createElement("div");
