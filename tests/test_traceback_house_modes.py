...
const { ctx } = H.makeCtx({ state: { _tracebackInitialMode: "insights" },
  wsCall: (t) => t === "padspan_bright/traceback_get" ? { frames, range: { start: 1000, end: 1010, count: 2 } }
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : { days: [], dwell: {}, entries: {}, occupancy: {}, objects: {} } });
...
const { ctx } = H.makeCtx({ states: { "binary_sensor.front_door": door },
  wsCall: (t) => t === "padspan_bright/traceback_get" ? { frames, range: { start: start + 10, end: start + 20, count: 2 } }
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {},
...
const { ctx } = H.makeCtx({ wsCall: (t) => t === "padspan_bright/traceback_get" ? { frames, range: { start: 1000, end: 1040, count: 5 } }
...
const { ctx, calls } = H.makeCtx({ states,
  wsCall: (t, d) => t === "padspan_bright/traceback_get" ? (objArgs.push(d.obj_key), { frames, range: { start: start + 10, end: start + 20, count: 2 } })
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {},
...
const { ctx, calls } = H.makeCtx({ states,
  wsCall: async (t, d) => t === "padspan_bright/traceback_get" ? (d.obj_key === undefined ? (await gate, { frames, range: {} }) : { frames, range: {} })
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {},
  callWS: (m) => m.type === "history/history_during_period" ? {} : {} });
ctx.state._traceback = undefined;
const outer = H.TB.render(ctx);
...
ctx.actions.wsCall = async (t, d) => { if (t === "padspan_bright/traceback_get") await gate2; return origWs(t, d); };
...
const { ctx } = H.makeCtx({ states,
  wsCall: (t) => t === "padspan_bright/traceback_get" ? { frames, range: { start: start + 10, end: start + 230, count: 12 } }
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {},
...
let hang = false;
const { ctx } = H.makeCtx({
  wsCall: (t, d) => t === "padspan_bright/traceback_get" ? (hang && d.obj_key === undefined ? new Promise(() => {}) : { frames, range: {} })
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {} });
ctx.state._traceback = undefined;
...
let gate = null;
const { ctx, calls } = H.makeCtx({ states,
  wsCall: async (t) => t === "padspan_bright/traceback_get" ? (gate && (await gate), { frames, range: {} })
    : t === "padspan_bright/traceback_objects" ? { objects: [] } : t === "padspan_bright/vacation_log_get" ? { actions: [], periods: [] } : {},
