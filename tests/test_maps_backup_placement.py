...
_BOTH = {MAPS_STORE_KEY: {"maps": []}, MODEL_STORE_KEY: {"map_transforms": {}},
         FABRIC_STORE_KEY: {"floors": {}}, "padspan_bright.settings": {}}
...
@pytest.mark.parametrize("selected,refused", [
    ...
    ([MAPS_STORE_KEY, "padspan_bright.settings"], True),
    ([MAPS_STORE_KEY, MODEL_STORE_KEY], False),
    (["padspan_bright.settings"], False),
    ([], False),
])
async def test_restoring_one_without_the_other_is_refused(
    ...
    """
    from custom_components.padspan_bright.ws_backup import ws_store_backup_restore
...
def test_the_panel_ties_the_two_checkboxes_together() -> None:
    ...
    assert 'c.value === "padspan_bright.maps"' in site, site[-800:]
    assert 'c.value === "padspan_bright.model"' in site
