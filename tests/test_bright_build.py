...
from custom_components.padspan_bright.build_info import EDITION  # noqa: E402
...
def test_the_working_tree_is_untouched(tmp_path):
    ...
    before = _git_status()
    src_before = {p: p.read_bytes() for p in (_ROOT / "custom_components" / "padspan_bright").rglob("*.py")}
    bb.build(tmp_path / "t", _ROOT)
    assert _git_status() == before
    assert (_ROOT / "custom_components" / "padspan_bright").is_dir()
...
def test_bright_exposes_no_entities():
    ...
    src = (_ROOT / "custom_components" / "padspan_bright" / "__init__.py").read_text(encoding="utf-8")
...
def test_verify_catches_a_surviving_name(tree, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="padspan_bright"):
        bb.verify(copy)
