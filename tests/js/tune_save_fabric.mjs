...
const ctx = {
  ...
  actions: {
    callWS: async (p) => {
      ...
      if (p.type === "padspan_bright/fabric_scanner_position_set") {
      ...
      if (p.type === "padspan_bright/fabric_scanner_z_set") {
      ...
      if (p.type === "padspan_bright/fabric_scanner_remove") {
...
function posPosts(list) {
  return list.filter(p => p.type === "padspan_bright/fabric_scanner_position_set");
}
...
async function flow_height_blocked() {
  ...
  J("height-pending", { heightCalls: calls.filter(
    c => c.type === "padspan_bright/fabric_scanner_z_set").length });
