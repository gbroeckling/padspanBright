...
from custom_components.padspan_bright.presence_rules import (
    DEFAULT_AWAY_TIMEOUT_M,
    away_timeout_s,
    is_away,
)

_ROOT = Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright"
...
class _Hass:
    def __init__(self, settings=None):
        from custom_components.padspan_bright.const import DATA_SETTINGS, DOMAIN
...
def test_no_file_hand_rolls_the_away_threshold():
    ...
    allowed = {
        ...
        _ROOT / "www" / "padspan-bright" / "views" / "settings.js",  # edits the setting
    }
    offenders: list[str] = []
...
def test_object_markers_cannot_stretch_the_overview_frame():
    ...
    """
    src = (_ROOT / "www" / "padspan-bright" / "views" / "overview.js").read_text(encoding="utf-8")
...
def test_no_surface_asserts_a_departed_object_is_still_in_a_room():
    ...
    """
    panel = (_ROOT / "www" / "padspan-bright" / "panel.js").read_text(encoding="utf-8")
    ...
    ov = (_ROOT / "www" / "padspan-bright" / "views" / "overview.js").read_text(encoding="utf-8")
