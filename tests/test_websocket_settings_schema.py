...
from custom_components.padspan_bright import websocket as ws
from custom_components.padspan_bright.settings_store import DEFAULT_SETTINGS


def _schema_keys() -> set[str]:
    """Declared keys of the padspan_bright/settings_set command schema."""
...
_BACKEND_ONLY = {
    "forensics_license_key",       # padspan_bright/forensics_license_activate
    "forensics_license_expires",   # ditto — derived at activation
    "license_is_trial",            # padspan_bright/trial_start — same reason, never user-settable
...
_WWW = Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright" / "www"
_CALL_RE = re.compile(r"settingsSet\s*\(\s*\{|padspan_bright/settings_set")
...
def _brace_object_start(src: str, m: "re.Match[str]") -> int:
    ...
    places relative to the match:
      ...
      2. "padspan_bright/settings_set", THEN a sibling {...} — the object is a
         separate, later call argument: wsCall("padspan_bright/settings_set",
         { ... }). Forward: skip the string's closing quote, whitespace and
         comma after the match, expect '{' immediately.
      3. "padspan_bright/settings_set" sitting INSIDE an object that already
         opened earlier — callWS({ type: "padspan_bright/settings_set", ... }).
