...
from custom_components.padspan_bright import ws_wled as W
...
async def test_a_cfg_write_backs_up_first_and_refuses_a_changed_device(fake):
    ...
    backup = Path(fake.tmp, "padspan_bright", "wled_backups", "aabbccddeeff", conn.results[0]["backup"])
...
async def test_a_restore_is_admin_only_and_takes_a_safety_copy_first(fake, monkeypatch):
    ...
    folder = Path(fake.tmp, "padspan_bright", "wled_backups", "aabbccddeeff", "20260101-000000")
...
async def test_a_restore_reads_only_this_devices_folder(fake, monkeypatch):
    ...
    other = Path(fake.tmp, "padspan_bright", "wled_backups", "112233445566", "20260101-000000")
