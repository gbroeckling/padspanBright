...
from custom_components.padspan_bright.fabric_store import FabricStore, _norm_geometry
...
async def test_history_appends_and_caps() -> None:
    from custom_components.padspan_bright import fabric_store as fs
    fab = _make_fabric()
    await fab.async_correct_room("main", "Kitchen", _SQUARE)
...
def test_no_implicit_spatial_write_path_survives() -> None:
    ...
    src = Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright"
