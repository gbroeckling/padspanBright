"""PadSpan Bright → PadSpan Bright import (bright_import.py).
...
def test_the_house_comes_across_and_the_entry_reloads(tmp_path):
    ...
    assert (tmp_path / "www" / "padspan_bright" / "maps" / "m1.png").read_bytes() == b"\x89PNG-fake"
    assert not (tmp_path / "www" / "padspan_bright" / "maps" / "notes.txt").exists()
