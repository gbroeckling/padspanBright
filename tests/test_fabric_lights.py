...
from custom_components.padspan_bright.const import DATA_MODEL, DOMAIN
from custom_components.padspan_bright.fabric_store import FabricStore
from custom_components.padspan_bright.migrations import _convert_lights
from custom_components.padspan_bright.model_store import ModelStore
from custom_components.padspan_bright.websocket import (
...
def _hass(mdl, *, pro: bool = True):
    ...
    from custom_components.padspan_bright.const import DATA_SETTINGS
    h = MagicMock()
...
async def test_a_lights_appearance_is_normalised_on_the_way_in() -> None:
    ...
    from custom_components.padspan_bright.model_store import light_appearance
...
def test_the_backend_shape_vocabulary_is_the_renderers() -> None:
    ...
    from custom_components.padspan_bright.model_store import LIGHT_PIN_SHAPES
    js = (Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright"
          / "www" / "padspan-bright" / "views" / "light_codes.js").read_text(encoding="utf-8")
...
def test_the_backend_type_override_vocabulary_is_the_choosers() -> None:
    ...
    import re
    from pathlib import Path
    from custom_components.padspan_bright.const import LIGHT_TYPE_OVERRIDE_KINDS
    js = (Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright"
          / "www" / "padspan-bright" / "views" / "light_codes.js").read_text(encoding="utf-8")
