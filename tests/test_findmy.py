...
from custom_components.padspan_bright import findmy as F
...
def _hass_for(labels=None, followed=None):
    ...
    from custom_components.padspan_bright.const import DATA_OBJECTS, DATA_SETTINGS, DOMAIN
    labels = labels or {}
...
async def test_every_address_a_moved_tag_used_maps_to_one_identity_and_follow_is_left_alone():
    from custom_components.padspan_bright import snapshot_builder as SB
...
async def test_an_unknown_tag_is_never_given_an_identity():
    ...
    from custom_components.padspan_bright.const import DOMAIN
    assert canon == {} and hass.data[DOMAIN]["findmy_bridge"].tags == {}


def test_the_merged_object_keeps_the_key_it_was_first_known_by():
    from pathlib import Path
    src = (Path(__file__).resolve().parents[1] / "custom_components" / "padspan_bright" / "snapshot_builder.py").read_text(encoding="utf-8")
...
async def test_the_unlink_command_answers_for_the_objects_key():
    ...
    from custom_components.padspan_bright import ws_objects as WO
    from custom_components.padspan_bright.const import DOMAIN
    b = F.FindMyBridge()
...
def test_a_replayed_advert_keeps_its_real_age(monkeypatch):
    ...
    import time
    from types import SimpleNamespace
    from custom_components.padspan_bright import bluetooth_live as BL
    from custom_components.padspan_bright.const import DOMAIN
    mono = time.monotonic()
