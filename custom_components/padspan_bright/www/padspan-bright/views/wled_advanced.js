...
export async function mountWledAdvanced(pane, { hass, eid, api }) {
  ...
  const get = (path) => call("padspan_bright/wled_get", { path }).then(r => r.data);
  const post = (body) => call("padspan_bright/wled_state", { body }).then(r => r.data);
...
function liveView(ctx, count, matrix) {
  ...
  const btn = h("button", { style: S.btn, onclick: async () => {
    ...
    try {
      ...
      ctx.liveUnsub = await conn.subscribeMessage((f) => ctx.onLive(f), { type: "padspan_bright/wled_live", entity_id: ctx.eid });
  ...
  const run = async () => {
    try {
      ...
      bar.appendChild(h("button", { style: S.btnPrimary + ";margin-left:6px", onclick: async () => {
        ...
        if (!viaPsave) {
          try {
            const cur = await ctx.call("padspan_bright/wled_get", { path: "json/cfg" });
            reportCfg(ctx, await ctx.call("padspan_bright/wled_cfg", { patch: { def: { ps: slot } }, base_hash: cur.hash }),
...
async function identify(ctx, seg) {
  try {
    await ctx.call("padspan_bright/wled_identify", { seg_id: seg.id, seconds: 10 });
