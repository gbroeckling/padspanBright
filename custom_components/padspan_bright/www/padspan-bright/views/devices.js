...
async function _loadRegistryAsync(ctx, el, container) {
  ...
  try {
    const res = await ctx.actions.callWS({ type: "padspan_bright/device_registry_list" });
    ...
    mergeBtn.addEventListener("click", async () => {
      ...
      try {
        await ctx.actions.callWS({ type: "padspan_bright/device_registry_merge", keep_id: ids[0], absorb_id: ids[1] });
    ...
    for (const d of entries) {
      ...
      lblInput.addEventListener("keydown", async (e) => {
        ...
        if (!newLabel) return;
        try {
          await ctx.actions.callWS({ type: "padspan_bright/device_registry_label_set", padspan_id: pid, label: newLabel });
      ...
      delBtn.addEventListener("click", async () => {
        ...
        try {
          await ctx.actions.callWS({ type: "padspan_bright/device_registry_delete", padspan_id: pid });
      ...
      addGo.addEventListener("click", async () => {
        ...
        try {
          await ctx.actions.callWS({ type: "padspan_bright/device_registry_add_identity", padspan_id: pid, kind: addKind.value, value: v });
