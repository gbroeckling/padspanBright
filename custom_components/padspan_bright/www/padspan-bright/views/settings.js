...
function _settingsPresence(ctx, el){
  ...
  // decides whether HA's registry gets to write into it.
  {
    ...
    sel.addEventListener("change", async () => {
      try {
        await ctx.actions.callWS({ type: "padspan_bright/fabric_sync_mode_set", mode: sel.value });
  ...
  previewBtn.addEventListener("click", async () => {
    ...
    try {
      const r = await ctx.actions.wsCall("padspan_bright/telemetry_preview");
  ...
  sendBtn.addEventListener("click", async () => {
    ...
    try {
      const r = await ctx.actions.wsCall("padspan_bright/telemetry_send_now");
  ...
  idBtn.addEventListener("click", async () => {
    idBtn.disabled = true;
    try {
      await ctx.actions.wsCall("padspan_bright/telemetry_reset_id");
  ...
  // ── Private BLE / IRK Management ─────────────────────────────────────────
  {
    ...
    autoBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await ctx.actions.wsCall("padspan_bright/irk_auto_detect", {});
        ...
        if (!found.length) {
          ...
          for (const item of newOnes) {
            ...
            addIrkBtn.addEventListener("click", async () => {
              ...
              try {
                await ctx.actions.wsCall("padspan_bright/private_ble_add_irk", { irk: item.irk_hex, name: item.name || "Phone" });
    ...
    async function _refreshIrkStatus() {
      try {
        const st = await ctx.actions.wsCall("padspan_bright/private_ble_status", {});
          ...
          for (const d of devices) {
            ...
            if (d.entry_id) {
              delBtn.addEventListener("click", async () => {
                ...
                delBtn.textContent = "...";
                try {
                  await ctx.actions.wsCall("padspan_bright/private_ble_delete_irk", { entry_id: d.entry_id });
                  await _refreshIrkStatus();
                } catch(e) {
    ...
    const _commitIrk = async (irk, name) => {
      try {
        const res = await ctx.actions.wsCall("padspan_bright/private_ble_add_irk", { irk, name: name || "PadSpan Device" });
        if (res.duplicate) {
    ...
    addBtn.addEventListener("click", async () => {
      ...
      for (let i = 0; i < rounds; i++) {
        ...
        try {
          lastResult = await ctx.actions.wsCall("padspan_bright/irk_validate", { irk_hex: irk });
...
`;
  ...
  if(adaptiveEnabled){
    (async()=>{
      try {
        const r = await ctx.actions.wsCall("padspan_bright/adaptive_status_get");
  ...
  resetAdaptiveBtn.addEventListener("click", async()=>{
    ...
    try {
      await ctx.actions.wsCall("padspan_bright/adaptive_reset");
  ...
  // ── Suspend Databases (raw radio test) ─────────────────────────────────
  {
    ...
    if (_isActive) {
      ...
      cancelBtn.addEventListener("click", async()=>{
        ...
        try {
          await ctx.actions.wsCall("padspan_bright/unsuspend_databases");
      ...
      suspendBtn.addEventListener("click", async()=>{
        ...
        try {
          await ctx.actions.wsCall("padspan_bright/suspend_databases",{minutes:60});
  ...
  // ── HA Tags Integration ─────────────────────────────────────────────────
  {
    ...
    (async () => {
      try {
        const res = await ctx.actions.wsCall("padspan_bright/tags_status", {});
...
function _tiersWizardFree(ctx, w, isBright){
  ...
  wrap.appendChild(el("div",{class:"muted", style:"margin-bottom:10px;line-height:1.6"},
    isBright
      ...
      : "This install is PadSpan Bright. Presence tracking, Overview, Follow, Pure Live, Occupancy, Calibration, mapping and everything else here is free, no key, forever."));
...
function _tiersWizardLadder(ctx, w, isBright){
  ...
  if (isBright) {
    ...
    wrap.appendChild(el("div",{class:"muted",style:"font-size:11.5px;margin-top:6px;line-height:1.5"},
      "Want presence tracking too? That's a separate, full download — PadSpan Bright — not an upgrade of this one."));
  } else {
    wrap.appendChild(row("PadSpan Bright (free)", null,
...
function _settingsLicence(ctx, el){
  ...
  const edition = String(s.edition || "full").toLowerCase() === "bright" ? "PadSpan Bright" : "PadSpan Bright";
  ...
  const enterBtn = el("button", { class: "btn inline", style: "font-size:12px",
    onclick: async () => {
      ...
      try {
        const r = await ctx.actions.wsCall("padspan_bright/forensics_license_activate", { key: key.trim() });
        if (r && r.ok) {
  ...
  if (!hasKey) {
    const trialBtn = el("button", { class: "btn inline", style: "font-size:12px;border-color:#52b788;color:#a7f3d0",
      onclick: async () => {
        ...
        try {
          const r = await ctx.actions.wsCall("padspan_bright/trial_start", { email: email.trim() });
          if (r && r.ok) {
            if (r.settings) ctx.state.settings = r.settings;
  ...
  if (hasKey) {
    const revealBtn = el("button", { class: "btn inline", style: "font-size:12px",
      onclick: async () => {
        try {
          const r = await ctx.actions.wsCall("padspan_bright/forensics_license_reveal", {});
...
function _settingsFeatures(ctx, el){
  ...
  // ── Forensics (privacy-sensitive, off by default — issue #55) ─────────────
  {
    ...
    (async()=>{
      try {
        const s = await ctx.actions.wsCall("padspan_bright/forensics_stats");
    ...
    delBtn.addEventListener("click", async()=>{
      ...
      try {
        const r = await ctx.actions.wsCall("padspan_bright/forensics_clear");
...
function _settingsUI(ctx, el){
  ...
  edCard.appendChild(el("div",{style:"font-size:12px;color:#94a3b8;margin-bottom:10px;line-height:1.5"},
    `This build: ${_ed === "bright" ? "PadSpan Bright" : "PadSpan Bright"} · running at tier "${_tier}"` +
