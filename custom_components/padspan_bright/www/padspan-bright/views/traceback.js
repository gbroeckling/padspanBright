...
export function render(ctx) {
  ...
  async function _loadTracebackData() {
    try {
      ...
      const res = await ctx.actions.wsCall("padspan_bright/traceback_get", {
        start_ts: startTs,
        end_ts: endTs,
      ...
      if (tb.filterKey && !tb.frames.length && tb.range && tb.range.start > 0) {
        const fullRes = await ctx.actions.wsCall("padspan_bright/traceback_get", {
      ...
      const objRes = await ctx.actions.wsCall("padspan_bright/traceback_objects", {});
