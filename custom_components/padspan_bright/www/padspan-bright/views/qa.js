...
export function render(ctx){
  ...
  const _renderPropHealth = (ph) => {
    ...
    if(ctx.state._qaExpandedProp){
      ...
      resetAdBtn.addEventListener("click", async()=>{
        ...
        try { await ctx.actions.wsCall("padspan_bright/adaptive_reset"); ctx.toast("Adaptive learning data cleared"); ctx.actions.renderRooms(); }
      ...
      resetCalBtn.addEventListener("click", async()=>{
        ...
        try { await ctx.actions.wsCall("padspan_bright/calibration_clear"); ctx.toast("All calibration data cleared"); ctx.actions.renderRooms(); }
        catch(e){ ctx.toast("Reset failed",true); }
  ...
  (async()=>{
    try {
      const r = await ctx.actions.wsCall("padspan_bright/propagation_health");
  ...
  createBkBtn.addEventListener("click", async()=>{
    ...
    try {
      ...
      await ctx.actions.wsCall("padspan_bright/store_backup_create", note ? {note} : {});
      noteInp.value = "";
  ...
  const _renderBackups = (list) => {
    ...
    for(const bk of list){
      ...
      restoreBtn.addEventListener("click", async()=>{
        ...
        try { await ctx.actions.wsCall("padspan_bright/store_backup_restore", {backup_id: bk.id}); ctx.toast("Backup restored — reloading"); setTimeout(()=>location.reload(), 1500); }
      ...
      delBtn.addEventListener("click", async()=>{
        ...
        try { await ctx.actions.wsCall("padspan_bright/store_backup_delete", {backup_id: bk.id}); ctx.toast("Backup deleted"); ctx.actions.renderRooms(); }
  ...
  (async()=>{
    try {
      const r = await ctx.actions.wsCall("padspan_bright/store_backup_list");
