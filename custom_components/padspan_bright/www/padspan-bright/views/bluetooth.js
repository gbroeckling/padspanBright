...
async function _addIrk(ctx, name, irk, msgEl, after, force = false) {
  ...
  try {
    const r = await ctx.actions.wsCall("padspan_bright/irk_add", { name, irk_hex: irk, force });
...
export function render(ctx) {
  ...
  _pbleHdr.addEventListener("click", () => {
    ...
    if (ctx.state._pbleExpanded && !ctx.state._pbleStatus) {
      ctx.actions.wsCall("padspan_bright/private_ble_status").then(res => {
        ...
        if (padspanDevs.length > 0) {
          ...
          for (const d of padspanDevs) {
            ...
            rmBtn.addEventListener("click", async (ev) => {
              ...
              await ctx.actions.wsCall("padspan_bright/irk_remove", { irk_hex: hex });
              ctx.state._pbleStatus = null;
              ctx.actions.wsCall("padspan_bright/private_ble_status").then(res => { ctx.state._pbleStatus = res; ctx.actions.renderRooms(); });
            });
...
function renderScanners(ctx, radios, sources, adsAll) {
  ...
  const row = r => {
    ...
    // Persisted via padspan_bright/scanner_offset_set WS call, applied during model inference.
    ...
    if (obj && obj.findmy && obj.key && obj.current_address
      ...
      card.appendChild(el("div", { style: "margin:6px 0 2px" }, [el("button", {
        ...
        onclick: async () => {
          ...
          try {
            await ctx.actions.wsCall("padspan_bright/findmy_unlink", { key: obj.key, address: selected });
...
function renderIrkPanel(ctx, snap) {
  ...
  validateBtn.addEventListener("click", async () => {
    ...
    try {
      const vRes = await ctx.actions.wsCall("padspan_bright/irk_validate", { irk_hex: irk });
      ...
      if (found.length === 0) {
        ...
        for (const f of found) {
          if (!f.already_registered && f.verified) {
            try {
              await ctx.actions.wsCall("padspan_bright/irk_add", { name: f.name || "Auto-detected", irk_hex: f.irk_hex });
            } catch(_e) {}
          }
        }
