      ...
      const undoBanner = el("div",{style:"margin-top:8px;padding:10px 14px;border-radius:8px;background:#2a1a0a;border:1px solid #d97706;display:flex;align-items:center;gap:10px;flex-wrap:wrap"},[
        ...
        el("button",{class:"btn danger", style:"font-size:12px", onclick:async ()=>{
          ...
          if(_mig.canvasExtended){
            try {
              await ctx.actions.callWS({ type:"padspan_bright/maps_revert_extend", map_id: _mig.targetMapId });
            } catch(e){ /* best effort */ }
          }
...
function _edit(ctx, map, allMaps){
  ...
  const _placeWall = async (name, material, atten, fracPts) => {
    ...
    try {
      const r = await ctx.actions.callWS({ type: "padspan_bright/fabric_rf_barrier_set", barrier: {
    ...
    if (ctx.state.maps._mode === "measure") {
      ...
      if (meas.length >= 2) {
        ...
        applyBtn.addEventListener("click", async () => {
          ...
          try {
            ...
            const _txResult = await ctx.actions.callWS({ type: "padspan_bright/fabric_map_transform_set", map_id: map.id, transform });
      ...
      if (_fabTx && _fabTx.scale_x_m) {
        ...
        reanchorBtn.addEventListener("click", async () => {
          ...
          try {
            const r = await ctx.actions.callWS({ type: "padspan_bright/fabric_map_reanchor", map_id: map.id });
        ...
        if(newId){
          ...
          if(bm.map_transform){
            await ctx.actions.callWS({
              type: "padspan_bright/fabric_map_transform_set",
...
// sees it — it is the only manual PadSpan Bright has ever shipped for the
...
function _help(ctx){
  ...
  if (edition !== "bright"){
    ...
    card.appendChild(el("div",{class:"muted", style:"margin-top:8px;line-height:1.5"},[
      "• Upload any floorplan image; the UI converts it to optimized PNG and stores it under /config/www/padspan_bright/maps/ so HA can serve it at /local/padspan_bright/maps/.",
      el("br"),
...
function _floorHeights(ctx){
  ...
  wrap.appendChild(el("div",{style:"margin-top:8px"}, el("button",{class:"btn inline", onclick: async ()=>{
    try{
      await ctx.actions.callWS({ type:"padspan_bright/fabric_floor_elevations_set", floors: rows.map(r => ({
...
function _stack(ctx, maps, helpBtn){
  ...
  for(const m of maps){
    ...
    tdSave.appendChild(el("button",{class:"btn inline", onclick: async ()=>{
      ...
      if(_fid && _fid !== OUTSIDE_FLOOR_ID){
        try{
          await ctx.actions.callWS({ type: "padspan_bright/fabric_floor_elevations_set", floors: [{
  ...
  const performSave = async (override) => {
    ...
    try {
      await ctx.actions.callWS({
        type: "padspan_bright/fabric_map_reanchor", map_id: tM.id,
...
// opens once per browser (localStorage padspan_bright_lights_tour_seen), the
...
function _lightsTourCard(ctx, wrap, paid){
  ...
  const close = () => {
    ...
    try { localStorage.setItem("padspan_bright_lights_tour_seen", "1"); } catch(e) {}
    ctx.actions.renderRooms();
  };
...
function _lightsTab(ctx, maps, active) {
  ...
  // (its coach mark, padspan_bright_lights_coach_seen), the builder's own
  ...
  let hintClosed = true;
  try { hintClosed = localStorage.getItem("padspan_bright_lights_hint_closed") === "1"; } catch(e) {}
    ...
    el("div", { class: "card-head", style: "display:flex;gap:10px;align-items:center;flex-wrap:wrap" }, [
      ...
      ...(hintClosed ? [] : [
        ...
        (() => {
          ...
          x.addEventListener("click", () => {
            try { localStorage.setItem("padspan_bright_lights_hint_closed", "1"); } catch(e) {}
            ctx.actions.renderRooms();
          });
          return x;
        })(),
  ...
  // mechanism as the sidebar's coach mark (padspan_bright_lights_coach_seen),
  ...
  if (!ctx.state._lightsTour && !ctx.state._lightsTourAutoChecked) {
    ...
    let seen = true;
    try { seen = localStorage.getItem("padspan_bright_lights_tour_seen") === "1"; } catch(e) {}
    if (!seen) {
      ctx.state._lightsTour = { step: 1 };
      if (ctx.actions.telemetryEvent) ctx.actions.telemetryEvent("lights_tour_opened");
    }
      ...
      el("button", { class: "btn inline primary lv-btn-big", onclick: async (e) => {
        ...
        try {
          for (const eid of dirtyEids) {
            ...
            await ctx.actions.wsCall("padspan_bright/fabric_light_position_set", { entity_id: eid, ...lp });
    ...
    if (entry) {
      ...
      insp.appendChild(el("button", {
        ...
        onclick: async () => {
          ...
          try {
            await ctx.actions.wsCall("padspan_bright/fabric_light_remove", { entity_id: sel.eid });
            await ctx.actions.modelRefresh();
...
// real space saved via padspan_bright/fabric_correct_room. Deliberately NOT
...
function _fetchRoomsTruth(ctx, floorId) {
  ...
  ctx.actions.wsCall("padspan_bright/fabric_truth_candidates", { floor_id: floorId })
  ...
  // in practice.
  {
    ...
    addRow.appendChild(el("button", {
      ...
      onclick: async (e) => {
        ...
        btn.disabled = true;
        try {
          await ctx.actions.wsCall("padspan_bright/fabric_room_add", { room: name, floor_id: floorId });
          ...
          await ctx.actions.wsCall("padspan_bright/fabric_correct_room", {
    ...
    if (_recon.length) {
      ...
      rRow.appendChild(el("button", {
        ...
        onclick: async (e) => {
          ...
          try {
            const res = await ctx.actions.wsCall("padspan_bright/fabric_rooms_reconcile", { floor_id: floorId });
    ...
    for (let bi = 0; bi < barDraft.length; bi++) {
      ...
      const del = el("div", {
        ...
        onclick: async (ev) => {
          ...
          try {
            await ctx.actions.wsCall("padspan_bright/fabric_rf_barrier_remove", { barrier_id: bar.id });
            mapState._roomsBarrierDraft = null;
            await ctx.actions.modelRefresh();
      ...
      if (sel.size === 1 && !previewing) {
        ...
        tools.appendChild(el("button", {
          ...
          onclick: async () => {
            ...
            try {
              await ctx.actions.wsCall("padspan_bright/fabric_room_remove", { room: only });
      ...
      const saveBtn = el("button", {
        ...
        onclick: async (e) => {
          ...
          for (const src of movedScanners) {
            ...
            try {
              await ctx.actions.wsCall("padspan_bright/fabric_scanner_position_set", {
          ...
          for (const key of movedBeacons) {
            const p = bkDraft[key];
            try {
              await ctx.actions.wsCall("padspan_bright/fabric_beacon_position_set", {
          ...
          for (const i of movedBarriers) {
            const b = barDraft[i];
            try {
              await ctx.actions.wsCall("padspan_bright/fabric_rf_barrier_set", {
                barrier: {
  ...
  if (!committed) {
    if (Object.keys(draft).length) {
      admin.appendChild(el("button", {
        ...
        onclick: async () => {
          ...
          try {
            await ctx.actions.wsCall("padspan_bright/fabric_floor_finalize", { floor_id: floorId, committed: true });
    ...
    admin.appendChild(el("button", {
      ...
      onclick: async () => {
        ...
        try {
          await ctx.actions.wsCall("padspan_bright/fabric_floor_finalize", { floor_id: floorId, committed: false });
