 ...
 *   the padspan_bright/system_critics WS handler on each render.
 */
...
export function render(ctx){
  ...
  // ── PadSpan Bright → PadSpan Bright import ──────────────────────────────────
...
async function _fetchAndRenderBrightImport(ctx, container) {
  const { el } = ctx.helpers;
  let st;
  try { st = await ctx.actions.callWS({ type: "padspan_bright/bright_import_status" }); }
  ...
  if (st.done_at) {
    card.appendChild(el("div",{style:"margin-top:8px;font-size:12px;color:#52b788"},
      `Imported into PadSpan Bright on ${new Date(st.done_at).toLocaleString()}. When the house looks right here, remove PadSpan Bright.`));
  ...
  card.appendChild(el("div",{class:"muted",style:"margin-top:8px;font-size:12px"},
    "Import copies Bright's floors, rooms, walls, placed lights, maps and settings into PadSpan Bright. "
    + "A backup of this install's stores is taken first (Backup / Restore). "
    + "It only runs into an empty house and never merges. PadSpan Bright reloads when it is done; "
  ...
  const btn = el("button",{class:"btn inline primary",style:"margin-top:8px",onclick: async () => {
    ...
    try {
      const r = await ctx.actions.callWS({ type: "padspan_bright/bright_import" });
      status.style.color = "#52b788";
      status.textContent = `Imported ${r.imported.join(", ")}${r.images ? ` and ${r.images} map image(s)` : ""} — backup ${r.backup_id}. PadSpan Bright is reloading; refresh in a moment.`;
    } catch (e) {
...
async function _fetchAndRenderFabric(ctx, container) {
  ...
  try {
    const res = await ctx.actions.callWS({ type: "padspan_bright/fabric_health" });
    _fabricCache = res;
...
function _renderFabric(ctx, container, data) {
  ...
  // ── Quick actions ───────────────────────────────────────────────────────
  {
    ...
    if (rfCheck) {
      ...
      rfBtn.addEventListener("click", async () => {
        ...
        try {
          const r = await ctx.actions.callWS({type:"padspan_bright/calibration_retrain_rf"});
    ...
    resyncBtn.addEventListener("click", async () => {
      ...
      try {
        const r = await ctx.actions.callWS({type:"padspan_bright/fabric_resync"});
    ...
    resetBtn.addEventListener("click", async () => {
      ...
      try {
        const r = await ctx.actions.callWS({type:"padspan_bright/fabric_reset_spatial"});
    ...
    if (_idCheck && _idCheck.value === "needed") {
      ...
      idBtn.addEventListener("click", async () => {
        ...
        try {
          const r = await ctx.actions.callWS({type:"padspan_bright/device_registry_migrate"});
  ...
  auditBtn.addEventListener("click", async () => {
    ...
    try {
      const res = await ctx.actions.callWS({type:"padspan_bright/radio_audit"});
...
async function _fetchAndRenderCritics(ctx, container) {
  ...
  try {
    const res = await ctx.actions.callWS({ type: "padspan_bright/system_critics" });
    _criticsCache = res;
...
function _renderCapture(ctx, container) {
  ...
  const refresh = async () => {
    ...
    let st;
    try { st = await ctx.actions.callWS({type:"padspan_bright/capture_status"}); }
    catch { return; }
    if (st.recording) {
  ...
  recBtn.addEventListener("click", async () => {
    ...
    try {
      const st = await ctx.actions.callWS({type:"padspan_bright/capture_status"});
      if (st.recording) {
        const r = await ctx.actions.callWS({type:"padspan_bright/capture_stop"});
        ...
        const r = await ctx.actions.callWS({type:"padspan_bright/capture_start", minutes:mins, label});
  ...
  markBtn.addEventListener("click", async () => {
    try {
      const msg = {type:"padspan_bright/capture_mark", room:markSel.value};
...
async function _capList(ctx, host) {
  const { el } = ctx.helpers;
  let list;
  try { list = (await ctx.actions.callWS({type:"padspan_bright/capture_list"})).sessions || []; }
  catch { return; }
  ...
  for (const s of list) {
    ...
    dl.addEventListener("click", async () => {
      ...
      try {
        ...
        const out = [];
        for (let off = 0; ; ) {
          const p = await ctx.actions.callWS({type:"padspan_bright/capture_get", session_id:s.id, offset:off, limit:2000});
          out.push(...p.lines);
          off += p.lines.length;
    ...
    del.addEventListener("click", async () => {
      ...
      try {
        await ctx.actions.callWS({type:"padspan_bright/capture_delete", session_id:s.id});
...
// roadmap). Fetches padspan_bright/capture_replay, shows room accuracy + metre
...
async function _renderReplayPanel(ctx, panel, sessionId) {
  ...
  let res;
  try {
    res = await ctx.actions.callWS({type:"padspan_bright/capture_replay", session_id: sessionId});
  } catch(e) {
  ...
  abBtn.addEventListener("click", async () => {
    ...
    try {
      const ab = await ctx.actions.callWS({
        type:"padspan_bright/capture_replay", session_id: sessionId, settings});
