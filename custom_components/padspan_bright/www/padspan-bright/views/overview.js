...
export function render(ctx){
  ...
  function renderIsoFloorStack(){
    ...
    if (_isoRadioMapOn || _isoDistortionOn) {
      ...
      if (!ctx.state._adaptiveFpLoaded) {
        ...
        ctx.actions.callWS({ type: "padspan_bright/adaptive_fingerprints_get" }).then(res => {
          if (res?.fingerprints) {
  ...
  const _buildDiagPanel = () => {
    ...
    hdr.addEventListener("click", async () => {
      ...
      try {
        const res = await ctx.actions.wsCall("padspan_bright/positioning_diag");
  ...
  if(isBasic){
    ...
    if (dataMode === "live") {
      ...
      (async () => {
        try {
          const res = await ctx.actions.wsCall("padspan_bright/companion_discover", {});
          ...
          for (const phone of phones) {
            ...
            if (!phone.has_irk && !phone.is_disabled) {
              ...
              irkSaveBtn.addEventListener("click", async (e) => {
                ...
                try {
                  await ctx.actions.wsCall("padspan_bright/irk_add", {
            ...
            const _setTrack = () => {
              ...
              btn.onclick = async () => {
                ...
                try {
                  const r = await ctx.actions.wsCall("padspan_bright/companion_follow", {
            ...
            const _setUnfollow = () => {
              ...
              btn.onclick = async () => {
                ...
                try {
                  await ctx.actions.wsCall("padspan_bright/companion_unfollow", {
                    ibeacon_key: phone.ibeacon_key,
                    device_name: phone.device_name,
                  });
              ...
              btn.addEventListener("click", async () => {
                ...
                try {
                  ...
                  if (phone.ibeacon_key) {
                    await ctx.actions.wsCall("padspan_bright/companion_follow", {
                      ibeacon_key: phone.ibeacon_key,
                      device_name: phone.device_name,
                      entity_id: phone.entity_id,
                    });
    ...
    el("div",{class:"card"},[
      ...
      unidentifiedCount > 0 ? el("div",{style:"margin-top:6px"},[
        el("button",{class:"btn inline",style:"font-size:11px;color:#f87171;border-color:#7f1d1d", onclick: async function(){
          ...
          try {
            const r = await ctx.actions.wsCall("padspan_bright/objects_clear_history",{});
  ...
  let occCard;
  {
    ...
    (async () => {
      try {
        _fill(await ctx.actions.callWS({type:"padspan_bright/occupancy_estimate"}));
      } catch(e) {
    ...
    occCard.addEventListener("click", async () => {
      try {
        const res = await ctx.actions.callWS({type:"padspan_bright/occupancy_estimate"});
        const col = _confColor(res.confidence);
        const ev = res.evidence || {};
        ...
        trainBtn.addEventListener("click", async () => {
          ...
          try {
            const tr = await ctx.actions.callWS({type:"padspan_bright/occupancy_train",actual_count:v});
