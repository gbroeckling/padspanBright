...
export function render(ctx){
  ...
  // just calibration data without overwriting their settings, for example.
  {
    ...
    const _storeLabel = (k) => {
      const map = {
        "padspan_bright.settings": "Settings",
        "padspan_bright.calibration": "Calibration",
        "padspan_bright.adaptive": "Adaptive Model",
        "padspan_bright.objects": "Objects",
        "padspan_bright.maps": "Maps Config",
        "padspan_bright.model": "Model",
        "padspan_bright.follow_alerts": "Alerts",
    ...
    const _renderBkList = async () => {
      ...
      try {
        const res = await ctx.actions.wsCall("padspan_bright/store_backup_list");
            ...
            btn.addEventListener("click", () => {
              ...
              // never has to meet that refusal.
              {
                const _pair = checkboxes.filter(
                  c => c.value === "padspan_bright.maps" || c.value === "padspan_bright.model");
                if(_pair.length === 2){
              ...
              confirmBtn.addEventListener("click", async()=>{
                ...
                try {
                  ...
                  const r = await ctx.actions.wsCall("padspan_bright/store_backup_restore", payload);
          ...
          const makeDelBtn = () => {
            ...
            btn.addEventListener("click", () => {
              ...
              yes.addEventListener("click", async()=>{
                delWrap.innerHTML = "";
                try {
                  await ctx.actions.wsCall("padspan_bright/store_backup_delete", {backup_id: bk.id});
    ...
    const makeCreateBtn = () => {
      ...
      btn.addEventListener("click", async () => {
        ...
        try {
          const r = await ctx.actions.wsCall("padspan_bright/store_backup_create", {note: "Manual backup"});
...
function _haEntitiesIntro(el){
  return el("div",{class:"card",style:"border-color:#1b4a2e"},[
    el("div",{style:"font-weight:800;font-size:15px;color:#5eead4;margin-bottom:6px"},"PadSpan Bright — Entity Reference"),
...
function _haEntityAudit(ctx, el){
  ...
  // Fire the WS call
  ctx.actions.wsCall("padspan_bright/ha_entities_audit", {}).then(res => {
    body.innerHTML = "";
...
function _haEspresenseSection(ctx, el, settings){
  ...
  importBtn.addEventListener("click", async()=>{
    ...
    try {
      const r = await ctx.actions.wsCall("padspan_bright/espresense_companion_import", {});
...
//   3. Stored object labels (padspan_bright.objects store)
// ══════════════════════════════════════════════════════════════════════════════
function _history(ctx, el){
  ...
  if(!ctx.state._manageMovementLoaded){
    ...
    ctx.actions.wsCall("padspan_bright/movement_history_get", {limit: 500}).then(r => {
...
function _buildNotifications(ctx, el){
  ...
  ctx.actions.wsCall("padspan_bright/notify_services_list", {}).then(r => {
  ...
  for(const obj of trackable){
    ...
    testBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        await ctx.actions.wsCall("padspan_bright/notify_test", payload);
...
function _logs(ctx, el) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const res = await ctx.actions.callWS({ type: "padspan_bright/logs_get", level: ctx.state._logsLevel, limit: 300 });
...
function _beaconChars(ctx, el){
  ...
  masterBtn.addEventListener("click", () => {
    ...
    ctx.actions.wsCall("padspan_bright/settings_set", {
  ...
  const refresh = () => {
    ...
    ctx.actions.renderRooms();
    ctx.actions.wsCall("padspan_bright/calibration_beacon_profiles").then(res => {
...
function _factoryReset(ctx, el){
  ...
  resetBtn.addEventListener("click", async ()=>{
    ...
    try {
      await ctx.actions.wsCall("padspan_bright/settings_set", { data_mode: "live" });
    ...
    try {
      const sRes = await ctx.actions.wsCall("padspan_bright/settings_get", {});
      if(sRes?.settings){
    ...
    try {
      const snapRes = await ctx.actions.wsCall("padspan_bright/live_snapshot", {});
      if(snapRes?.snapshot){
