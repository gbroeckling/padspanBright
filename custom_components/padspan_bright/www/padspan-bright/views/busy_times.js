 ...
 *
 * No new backend: reuses padspan_bright/insights_get verbatim (same call
 ...
 * padspan_bright/insights_get Insights uses), so this is a presentation-layer
  ...
  const load = async (fresh) => {
    ...
    if (!_loading) {
      ...
      _loading = (async () => {
        _error = null;
        try {
          const res = await ctx.actions.wsCall("padspan_bright/insights_get", { days: _days });
