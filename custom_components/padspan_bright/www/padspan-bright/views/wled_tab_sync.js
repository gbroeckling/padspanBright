...
async function deviceCfg(ctx, deviceId) {
  const r = await ctx.hass.callWS({ type: "padspan_bright/wled_get", device_id: deviceId, path: "json/cfg" });
  return { cfg: r.data || {}, hash: r.hash };
}
async function deviceInfo(ctx, deviceId) {
  return (await ctx.hass.callWS({ type: "padspan_bright/wled_get", device_id: deviceId, path: "json/info" })).data || {};
}
async function deviceState(ctx, deviceId, body) {
  return ctx.hass.callWS({ type: "padspan_bright/wled_state", device_id: deviceId, body });
}
...
async function deviceCfgWrite(ctx, deviceId, patch, baseHash) {
  ...
  return ctx.hass.callWS({ type: "padspan_bright/wled_cfg", device_id: deviceId, patch, base_hash: hash });
}
...
async function savedSyncCard(ctx, host) {
  ...
  let cfg, hash;
  try { ({ data: cfg, hash } = await ctx.call("padspan_bright/wled_get", { path: "json/cfg" })); }
  ...
  if (ctx.isAdmin) card.appendChild(h("button", { style: S.btnPrimary + ";margin-top:8px", onclick: async () => {
    try {
      const r = await ctx.call("padspan_bright/wled_cfg", { patch: { if: { sync: { send, recv } } }, base_hash: hash });
...
async function teamCard(ctx, host) {
  ...
  try {
    ...
    [got, { devices }] = await Promise.all([
      ctx.hass.callWS({ type: "padspan_bright/wled_teams_get" }),
      ctx.hass.callWS({ type: "padspan_bright/wled_devices" }),
    ]);
  ...
  const saveTeams = async (list) => {
    const r = await ctx.hass.callWS({ type: "padspan_bright/wled_teams_set", teams: list, ...(listHash ? { base_hash: listHash } : {}) });
