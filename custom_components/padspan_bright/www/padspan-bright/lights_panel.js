...
/*
  PadSpan Bright — Lights Control Panel
  ===================================
...
// ── Persistence keys ─────────────────────────────────────────────────────────
const LS_HIDDEN = "padspan_bright_lights_hidden";
...
class PadSpanLightsApp extends HTMLElement {
  ...
  _saveHidden(){
    ...
    if(this._hass){
      try{ this._hass.callWS({ type:"padspan_bright/settings_set", lights_hidden: arr }); }catch(_){}
    }
  }
  ...
  async _loadModel(){
    try{
      const res = await this._hass.callWS({ type:"padspan_bright/model_get" });
  ...
  async _loadSettings(applyView=true){
    ...
    try{
      const res = await this._hass.callWS({ type:"padspan_bright/settings_get" });
      s = res?.settings || {};
    }catch(e){}
    try{
      if(applyView){
  ...
  _gotoBuilder(eid){
    ...
    try{ window.location.assign(`/padspan-bright?${q.toString()}`); }catch(_){}
  }
  ...
  _buildUI(){
    ...
    const host={
      ...
      onVacationModeDisable: async () => {
        if (!this._hass) return false;
        try { await this._hass.callWS({ type: "padspan_bright/settings_set", vacation_mode_enabled: false }); }
      ...
      onVacationModeIntensity: async (pct) => {
        ...
        try { await this._hass.callWS({ type: "padspan_bright/settings_set", vacation_mode_intensity: pct }); }
...
if (!customElements.get("padspan-bright-lights-app")) {
  customElements.define("padspan-bright-lights-app", PadSpanLightsApp);
}
