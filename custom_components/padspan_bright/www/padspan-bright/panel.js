...
PadSpan Bright Panel (single sidebar entry).
...
function mapImageUrl(map){
  ...
  return "/local/padspan_bright/maps/" + fn + (v ? "?v=" + v : "");
}
...
// PadSpanHaApp is registered as a custom element ("padspan-bright-app") and mounted
...
class PadSpanHaApp extends HTMLElement {
  ...
  connectedCallback(){
      ...
      <div id="app" class="app">
        ...
        <aside class="left">
          <div class="sidebar-mobile-header" id="sidebarMobileHeader">
            <span style="font-weight:700;font-size:15px;flex:1">PadSpan Bright</span>
            <button class="btn inline" id="sidebarClose" style="width:auto;font-size:18px;padding:4px 10px">&times;</button>
          </div>
          <div class="brand">
            <img src="/padspan_bright_static/padspan-bright/assets/padspan-mark.svg?b=${BUILD_ID}" alt="PadSpan" onerror="this.style.display='none'">
            <div>
    ...
    // would otherwise stack up duplicate listeners.
    {
      ...
      slider.addEventListener("change", async () => {
        try { await this._callWS({ type: "padspan_bright/settings_set", vacation_mode_intensity: parseInt(slider.value, 10) }); }
        catch(e){ this._toast("Could not change the intensity: " + String(e), true); }
      });
      this.$("#vacationBannerDisable").addEventListener("click", async () => {
        try { await this._callWS({ type: "padspan_bright/settings_set", vacation_mode_enabled: false }); await this._loadSettings(); }
    ...
    // /padspan-bright?view=purelive opens that view regardless of the browser's
    ...
    if(Object.keys(VIEWS).length > 0){
      ...
      if(this.$content){
        ...
        lo.innerHTML = `<div style="margin-bottom:16px"><svg width="48" height="48" viewBox="0 0 48 48"><circle cx="24" cy="24" r="20" fill="none" stroke="#1b3526" stroke-width="4"/><circle cx="24" cy="24" r="20" fill="none" stroke="#52b788" stroke-width="4" stroke-linecap="round" stroke-dasharray="80 45"><animateTransform attributeName="transform" type="rotate" from="0 24 24" to="360 24 24" dur="1s" repeatCount="indefinite"/></circle></svg></div>`
          + `<div style="color:#52b788;font-size:16px;font-weight:700;margin-bottom:6px">PadSpan Bright v${APP_VERSION}</div>`
  ...
  _whatsNewCard(){
    ...
    const remember = (toast) => {
      this._callWS({ type: "padspan_bright/settings_set", whatsnew_seen_version: APP_VERSION })
    ...
    card.appendChild(el("div", { style: "font-weight:700;font-size:13px;color:#52b788;margin-bottom:4px" },
      `PadSpan Bright updated to v${APP_VERSION}`));
  ...
  _telemetryAskCard(compact){
    ...
    const decide = async (on) => {
      try {
        ...
        const res = await this._callWS(Object.assign({ type: "padspan_bright/settings_set" },
    ...
    prev.addEventListener("click", async () => {
      prev.disabled = true;
      try {
        const r = await this._callWS({ type: "padspan_bright/telemetry_preview" });
  ...
  async _loadAlertConfigs(){
    try{
      const res = await this._callWS({ type: "padspan_bright/follow_alert_get" });
  ...
  _applySkin(){
    ...
    if (link) {
      if (skin === "2025") {
        link.href = `/padspan_bright_static/padspan-bright/styles-2025.css?v=${APP_VERSION}&b=${BUILD_ID}`;
      } else {
  ...
  _telemetryEvent(name){
    ...
    this._callWS({ type: "padspan_bright/telemetry_event", event: String(name) }).catch(() => {});
  }
  ...
  async _setDataMode(mode){
    try {
      const res = await this._callWS({ type: "padspan_bright/settings_set", data_mode: mode });
  ...
  async _getVersionInfo(){
    try {
      const res = await this._callWS({ type: "padspan_bright/version" });
  ...
  async _getStatus(){
    const res = await this._callWS({ type: "padspan_bright/status" });
  ...
  async _getRoomTags(){
    const res = await this._callWS({ type: "padspan_bright/room_tags" });
  ...
  async _getLiveSnapshot(){
    ...
    try {
      const res = await this._callWS({ type: "padspan_bright/live_snapshot" });
  ...
  async _getMapsList(retry = true){
    ...
    try {
      const res = await this._callWS({ type: "padspan_bright/maps_list" });
  ...
  async _getModel(){
    try {
      const res = await this._callWS({ type: "padspan_bright/model_get" });
  ...
  async _runAutoDiag(userAction=false){
    try {
      ...
      const res = await this._callWS({ type: "padspan_bright/auto_diagnostics" });
      this.state.diag = res;
      ...
      this.state.diag = {
        ...
        recommendations: ["Check Home Assistant logs for padspan_bright errors."],
      };
    ...
    return {
      ...
      actions: {
        ...
        vendorLookup: async (mac, force_refresh=false)=>{
          return await this._callWS({ type:"padspan_bright/vendor_lookup", mac, force_refresh: !!force_refresh });
        },

        // Object label actions (tag/untag BLE devices)
        objectLabelSet: async (address, label)=>{
          const res = await this._callWS({ type:"padspan_bright/object_label_set", address, label });
        ...
        objectLabelDelete: async (address)=>{
          return await this._callWS({ type:"padspan_bright/object_label_delete", address });
        },
        objectLabelList: async ()=>{
          return await this._callWS({ type:"padspan_bright/object_label_list" });
        },
        tagObjectPrompt: (addr, currentLabel)=>this._tagObjectPrompt(addr, currentLabel),
        radioAreaSet: async (payload)=>await this._callWS({ type:"padspan_bright/radio_area_set", ...payload }),
        radioLostSet: async (source, lost)=>await this._callWS({ type:"padspan_bright/radio_lost_set", source, lost }),
        radioDisabledSet: async (source, disabled)=>await this._callWS({ type:"padspan_bright/radio_disabled_set", source, disabled }),
        // radioReset: full reset + re-fetch + re-render (use for user-initiated resets)
        radioReset: async (source)=>{ const r = await this._callWS({ type:"padspan_bright/radio_reset", source }); await this._getLiveSnapshot(); await this._loadSettings(); this._scheduleRender(); return r; },
        ...
        radioResetQuiet: async (source)=>{ return await this._callWS({ type:"padspan_bright/radio_reset", source }); },
        ...
        followAlertSave: async (payload)=>await this._callWS({ type:"padspan_bright/follow_alert_save", ...payload }),
        followAlertDelete: async (addr)=>{
          await this._callWS({ type:"padspan_bright/follow_alert_delete", addr });
        ...
        areaDelete: async (area_id) =>
            await this._callWS({ type: "padspan_bright/area_delete", area_id }),
        entityDelete: async (entity_id) =>
            await this._callWS({ type: "padspan_bright/entity_delete", entity_id }),
        roomTagPurgeMissing: async () =>
            await this._callWS({ type: "padspan_bright/room_tag_purge_missing" }),
        integrationReload: async () =>
            await this._callWS({ type: "padspan_bright/integration_reload" }),
        factoryReset: async () => {
            const res = await this._callWS({ type: "padspan_bright/factory_reset", confirm: "FACTORY RESET" });
        ...
        mapsDelete: async (id)=>{ await this._callWS({ type:"padspan_bright/maps_delete", map_id:id }); await this._getMapsList(); if(this.state.activeMapId===id) this.state.activeMapId=null; this._scheduleRender(); },
        mapsDeleteMigrate: async (mapId, targetMapId, extendCanvas=false)=>{ const r = await this._callWS({ type:"padspan_bright/maps_delete_migrate", map_id:mapId, target_map_id:targetMapId, extend_canvas:!!extendCanvas }); await this._getMapsList(); if(this.state.activeMapId===mapId) this.state.activeMapId=null; this._scheduleRender(); return r; },
        mapsUpload: async (payload)=>{ const r = await this._callWS(Object.assign({type:"padspan_bright/maps_upload"}, payload)); await this._getMapsList(); return r; },
        mapsUpdate: async (payload)=>{ await this._callWS(Object.assign({type:"padspan_bright/maps_update"}, payload)); await this._getMapsList(); this._scheduleRender(); },
        mapsUpdateQuiet: async (payload)=>{ await this._callWS(Object.assign({type:"padspan_bright/maps_update"}, payload)); },
        mapsRefreshQuiet: async ()=>{ await this._getMapsList(); },
        mapsReplaceImage: async (payload)=>{ const res = await this._callWS(Object.assign({type:"padspan_bright/maps_replace_image"}, payload)); await this._getMapsList(); this._scheduleRender(); return res; },
        modelUpdate: async (payload)=>{ await this._callWS(Object.assign({type:"padspan_bright/model_update"}, payload)); await this._getModel(); this._scheduleRender(); },
        ...
        settingsSet: async (payload) => {
          ...
          const res = await this._callWS(Object.assign({ type: "padspan_bright/settings_set" }, payload));
          this.state.settings = res?.settings || this.state.settings;
          this._applyTheme();
          this._applySkin();
          this._renderNav();
        ...
        scannerOffsetSet: async (source, offset_db) => {
          const res = await this._callWS({ type: "padspan_bright/scanner_offset_set", source, offset_db });
          await this._getLiveSnapshot();
          await this._loadSettings();
          return res;
        },
        // BLE calibration actions
        calibrationGet: async () => await this._callWS({ type: "padspan_bright/calibration_get" }),
        calibrationSavePoint: async (point) => await this._callWS({ type: "padspan_bright/calibration_save_point", point }),
        calibrationDeletePoint: async (point_id) => await this._callWS({ type: "padspan_bright/calibration_delete_point", point_id }),
        calibrationClear: async () => await this._callWS({ type: "padspan_bright/calibration_clear" }),
        calibrationClearMap: async (map_id) => await this._callWS({ type: "padspan_bright/calibration_clear_map", map_id }),
        objectEvict: async (key) => await this._callWS({ type: "padspan_bright/object_evict", key }),
        calibrationComputeModel: async () => await this._callWS({ type: "padspan_bright/calibration_compute_model" }),
        calibrationSwapRadio: async (old_source, new_source) => await this._callWS({ type: "padspan_bright/calibration_swap_radio", old_source, new_source }),
        calibrationRelearnRadio: async (source, gain_db) => {
          const res = await this._callWS({ type: "padspan_bright/calibration_relearn_radio", source, gain_db });
          await this._getLiveSnapshot();
          return res;
        },
        calibrationHealthCheck: async () => await this._callWS({ type: "padspan_bright/calibration_health_check" }),
        ...
        followedToggle: (addr) => {
          ...
          this._callWS({
            type: "padspan_bright/settings_set",
  ...
  _tagObjectPrompt(addr, currentLabel){
    ...
    saveBtn.addEventListener("click", async ()=>{
      ...
      try {
        await this._callWS({ type:"padspan_bright/object_label_set", address: addr, label });
    ...
    clearBtn.addEventListener("click", async ()=>{
      try {
        await this._callWS({ type:"padspan_bright/object_label_delete", address: addr });
  ...
  _showObjectDetail(obj){
    ...
    if(canRename){
      ...
      saveRenameBtn.addEventListener("click", async()=>{
        ...
        try {
          await this._callWS({ type:"padspan_bright/object_label_set", address: tagAddr, label });
          this._closeModal();
      ...
      if(userLabel){
        ...
        untagBtn.addEventListener("click", async()=>{
          try {
            await this._callWS({ type:"padspan_bright/object_label_delete", address: tagAddr });
            this._closeModal();
            this._toast("Label removed.");
            await this._getLiveSnapshot();
            this._scheduleRender();
    ...
    if(_followKey || canRename){
      ...
      deleteBtn.addEventListener("click", async()=>{
        ...
        if(canRename && tagAddr){
          try { await this._callWS({ type:"padspan_bright/object_label_delete", address: tagAddr }); } catch(e){}
        }
        this._closeModal();
  ...
  _showScannerDetail(scanner){
    ...
    lostBtn.addEventListener("click", async ()=>{
      ...
      try {
        await this._callWS({ type:"padspan_bright/radio_lost_set", source: scanner.source||"", lost: !scanner.lost });
        this._closeModal();
    ...
    disabledBtn.addEventListener("click", async ()=>{
      ...
      try {
        await this._callWS({ type:"padspan_bright/radio_disabled_set", source: scanner.source||"", disabled: !scanner.disabled });
        this._closeModal();
...
// throws "name 'padspan-bright-app' has already been used", which aborts panel
...
if (!customElements.get("padspan-bright-app")) {
  customElements.define("padspan-bright-app", PadSpanHaApp);
}
