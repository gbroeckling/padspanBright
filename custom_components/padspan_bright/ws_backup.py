...
@websocket_api.websocket_command({
    "type": "padspan_bright/store_backup_restore",
...
async def ws_store_backup_restore(hass: HomeAssistant, connection, msg) -> None:
    ...
    Map images are restored to www/padspan_bright/maps/ with path traversal protection.
    """
    from homeassistant.helpers.storage import Store as _St
    ...
    #
    # A map's picture and stack live in `padspan_bright.maps`; where that map SITS,
    # in metres, lives in `padspan_bright.model` under `map_transforms[map_id]`,
    ...
    # the loop below rolls padspan_bright.model back while leaving the CURRENT
