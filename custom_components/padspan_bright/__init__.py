...
"""
PadSpan Bright — Integration Entry Point
======================================
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def _dump_devices(call: ServiceCall) -> dict | None:
        ...
        try:
            ...
            async_create(
                hass,
                f"**PadSpan Bright — {len(data)} device(s)** (v{BUILD_VERSION})\n\n{summary}",
                title="padspan_bright.dump_devices",
                notification_id="padspan_bright_dump_devices",
            )
        except Exception:
            pass
