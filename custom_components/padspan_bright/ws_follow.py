...
@websocket_api.websocket_command({"type": "padspan_bright/room_tags"})
...
@websocket_api.websocket_command({"type": "padspan_bright/follow_alert_get"})
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        "type": "padspan_bright/follow_alert_save",
...
async def ws_follow_alert_save(hass: HomeAssistant, connection, msg) -> None:
    ...
    _LOGGER.debug("PadSpan Bright follow_alert_save: addr=%s keys=%s", addr, list(config.keys()) if isinstance(config, dict) else "?")
    connection.send_result(msg["id"], {"ok": True, "addr": addr})


@websocket_api.websocket_command(
    {
        "type": "padspan_bright/follow_alert_delete",
...
async def ws_follow_alert_delete(hass: HomeAssistant, connection, msg) -> None:
    ...
    if alert_store:
        ...
        if addr in alerts:
            del alerts[addr]
            deleted = True
    _LOGGER.debug("PadSpan Bright follow_alert_delete: addr=%s deleted=%s", addr, deleted)
    connection.send_result(msg["id"], {"ok": True, "addr": addr, "deleted": deleted})


@websocket_api.websocket_command(
    {
        "type": "padspan_bright/area_delete",
        "area_id": str,
    }
)
...
@websocket_api.websocket_command(
    {
        "type": "padspan_bright/entity_delete",
        "entity_id": str,
    }
)
@websocket_api.require_admin
...
@websocket_api.websocket_command({"type": "padspan_bright/room_tag_purge_missing"})
@websocket_api.async_response
