...
"""PadSpan Bright websocket API — registration and the handlers with no subject of their own.
...
registers everything (async_register_websockets), so the panel sees one API.
    ...
    ws_bright_import   the PadSpan Bright → PadSpan Bright import
...
@websocket_api.websocket_command({"type": "padspan_bright/status"})
...
@websocket_api.websocket_command({"type": "padspan_bright/version"})
@websocket_api.async_response
...
@websocket_api.websocket_command({"type": "padspan_bright/model_get"})
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        "type": "padspan_bright/model_update",
...
@websocket_api.websocket_command(
    {
        "type": "padspan_bright/vendor_lookup",
        "mac": str,
...
@websocket_api.websocket_command({"type": "padspan_bright/integration_reload"})
@websocket_api.require_admin
@websocket_api.async_response
async def ws_integration_reload(hass: HomeAssistant, connection, msg) -> None:
    """Reload the PadSpan Bright config entry."""
    reloaded = 0
    for entry in hass.config_entries.async_entries(DOMAIN):
        try:
            ...
            _LOGGER.warning("PadSpan Bright reload failed for %s: %s", entry.entry_id, e)
...
@websocket_api.websocket_command({
    "type": "padspan_bright/movement_history_get",
...
@websocket_api.websocket_command({
    "type": "padspan_bright/traceback_get",
...
@websocket_api.websocket_command({"type": "padspan_bright/traceback_objects"})
@websocket_api.async_response
...
@websocket_api.websocket_command({
    "type": "padspan_bright/vacation_log_get",
...
@websocket_api.websocket_command({
    "type": "padspan_bright/suspend_databases",
...
@websocket_api.websocket_command({"type": "padspan_bright/unsuspend_databases"})
@websocket_api.async_response
...
@websocket_api.websocket_command({"type": "padspan_bright/tags_status"})
@websocket_api.async_response
