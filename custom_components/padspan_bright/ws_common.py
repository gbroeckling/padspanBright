...
# Captures WARNING+ from all padspan_bright loggers so the UI can show them.
...
class _RingLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        mod = record.name[len("custom_components.padspan_bright"):].lstrip(".") \
            if record.name.startswith("custom_components.padspan_bright") else record.name
        if not mod:
            mod = "__init__"
        entry = {
            "ts": record.created,
...
def _ensure_log_handler() -> _RingLogHandler:
    ...
    if _log_handler is None:
        ...
        # Attach to the padspan_bright root logger to capture all sub-modules
        root = logging.getLogger("custom_components.padspan_bright")
...
def _get_settings(hass: HomeAssistant) -> dict:
    ...
    it explicitly via padspan_bright/forensics_license_reveal.
    """
    st = hass.data.get(DOMAIN, {}).get(DATA_SETTINGS)
    if not st:
